...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            onErrorType: function(files,response){},
            ...
            dragDropStr: "<span><b>Arrastrar y soltar archivo(s) <i class='fas fa-arrow-down'></i></b></span>",
            uploadStr:"Subir <i class='fas fa-cloud-upload-alt'></i>",
            abortStr: "Abortar",
            cancelStr: "<i class='fas fa-ban'></i> Cancelar",
            deleteStr: "Eliminar",
            doneStr: "Terminado",
            multiDragErrorStr: "<div class='alert alert-warning' role='alert'>La opción de arrastrar y soltar archivos múltiples no está permitida.</div>",
            extErrorStr: "no es una extensión valida. Extensiones permitidas: ",
            duplicateErrorStr: "no es permitido. El archivo ya existe.",
            sizeErrorStr: "no es permitido. El tamaño máximo permitido es: ",
            uploadErrorStr: "<div class='alert alert-danger' role='alert'>Error al subir el archivo</div>",
            maxFileCountErrorStr: " no es permitido. El límite de archivos permitidos es: ",
            ...
            ddObj.on('drop', function (e) {
                ...
                for(var i = 0; i < files.length; i++) {
                    if(!isFileTypeAllowed(obj, s, files[i].name)) {
                        ...
                        s.onErrorType(files,'Error Types');
                        return;
                    }                                      
                }
                ...
                if(this.files) //support reading files
                {
                    for(i = 0; i < this.files.length; i++) {
                        ...
                        if(!isFileTypeAllowed(obj, s, this.files[i].name)) {
                            if(s.showError) $("<div class='" + s.errorClass + "'><b>" + this.files[i].name + "</b> " + s.extErrorStr + s.allowedTypes + "</div>").appendTo(
                                obj.errorLog);
                                s.onErrorType(files,'Error Types');
                            return;
                        }
